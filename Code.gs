...
function rbAssignHtml(iso) {
  try {
    ...
    var rows = an.rows.map(function (r) {
      ...
      var zp = String(r.zones || '0/0/0').split('/');       // ในกะ/OT/นอกกะ (แยกคอลัมน์)
      var zcell = '<td class="tnum ' + ((+zp[0] > 0) ? 'okk' : 'muted') + '">' + (zp[0] || '0') + '</td>' +
        '<td class="tnum" style="' + ((+zp[1] > 0) ? 'color:#c2410c;font-weight:700' : 'color:#9aa') + '">' + (zp[1] || '0') + '</td>' +
...
function rbOTAheadHtml(iso) {
  try {
    ...
    D.persons.forEach(function (p) {
      ...
      // สรุป "ช่วงนี้" แยก ปกติ vs OT ให้ชัด (ชั่วโมง + จำนวนไฟลท์)
